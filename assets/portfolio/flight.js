...
//Variables for the Phong Shading model
...
/*Function to initialize buffers and send them for rendering*/
function draw(){
...
function generateTerrain(x, z){
	...
	//Taking the generated values and pushing them onto the vertices array	
	while (minZ <= maxZ){
		i = minZ;
		minX = x[0];
		while (minX <= maxX){
	...
	console.log(vertices.slice(0,10))
	for (var k=2; k<vertices.length-2; k++){
		vertices[k-2][1] = noise.perlin2(vertices[k-2][0], vertices[k-2][2]);
		//If the user has selected a smooth shading it will compute the normals at those vertices and store it onto
		//the faces dictionary
		if (smooth_shading){
			facesDict[vertices[k-2]] = computeNormal(vertices[k-2], vertices[k-1], vertices[k]);	
		}
		//Pushing the colors according to their vertices if they are above the ground level or below the ground level
	...
	//For both flat_shading and smooth_shading the normals are generated
	if (flat_shading){
		generateNormals(vertices, 0, vertices.length);
	}
	else if (smooth_shading){
		var result = vec4(0,0,0,0);
		var avg, val1, val2, val3, val4;
...
// A function to compute the normals for smooth shading 
...
//Normals are computed per triangle for flat shading
...
//The render function is called recursively and draws the scene
function render() {
	...
	// Sets the scene according to whether it is wireframe.
	if (wireframe){
...
//This function is for checking the keypresses and making the appropriate function calls
...
function updateMovement() {
	...
	if (keysPressed[39])	// Yaw right
		yawRight();
	if (keysPressed[187])   // Accelerate 
		accelerate();
	if (keysPressed[189])   // Decelerate
		decelerate();
}